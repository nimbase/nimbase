...
proc buildModule(v: Values, extName, outputExt: string) =
  # the actual `build` thing, executing `nim c --app:lib`  with `-d:nimbaseExtension`
  ...
  let cmd = &"nim c --app:lib -d:nimbaseExtension --out:{outputFile} {modulePath}"
  let code = execCmd(cmd)
  if code != 0:
    displayError(&"Build failed for {extName} extension (exit code: {code})")

proc extensionCommand*(v: Values) =
  ## Build Nim as a native extension to a target high-level programming language
